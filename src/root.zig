...
pub const Wtype = struct {
    ...
    /// Add a text command to type the given text with optional delay between keystrokes
    /// @param text UTF-8 text string to type
    /// @param delay_ms Delay in milliseconds between each keystroke (0 for no delay)
    pub fn addTextCommand(self: *Wtype, text: []const u8, delay_ms: u32) !void {
        ...
        while (iter.nextCodepoint()) |codepoint| {
            ...
            const key_code = try self.getKeyCodeByWchar(codepoint);
            try key_codes.append(key_code);
        }

        // Create and add the text command
        const cmd = WtypeCommand{
            .type = .text,
            .data = .{ .text = .{ .key_codes = try key_codes.toOwnedSlice(), .delay_ms = delay_ms } },
        };
